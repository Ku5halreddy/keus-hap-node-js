...
// Called when an iOS client wishes to query the state of one or more characteristics, like "door open?", "light on?", etc.
  _handleGetCharacteristics =async  (data: CharacteristicData[], events: CharacteristicEvents, callback: HandleGetCharacteristicsCallback, remote: boolean, session: Session) => {

    // build up our array of responses to the characteristics requested asynchronously
    console.log("--->Got homekit sync request")
    var characteristics: CharacteristicData[] = [];
    var statusKey = remote ? 's' : 'status';
    var valueKey = remote ? 'v' : 'value';
    await this.emit(AccessoryEventTypes.GET_CHARACTERISTICS /* LISTENING */, function(chars: CharacteristicData[]){
